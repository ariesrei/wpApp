...
class thesis_asset_handler {
	...
	public function sitewide_nag() {
		global $thesis;
		if (($data = get_transient('thesis_core_update')) && is_super_admin() && version_compare($thesis->version, $data['new_version'], '<')) {
	...
	public function get_all_updates() {
		...
		$post_args = array(
			'body' => array(
				...
				'php' => phpversion(),
				'user-agent' => "WordPress/{$GLOBALS['wp_version']};" . site_url()
			)
		);
...
class thesis_generate extends WP_Upgrader {
	...
	private function create_seed() {
		...
		if (! $wp_filesystem->put_contents($skin_dir . 'seed.txt', $data))
