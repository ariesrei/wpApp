...
class thesis_box {
	...
	public function _class_canvas() {
		...
		echo
            ...
            $options['output'],
			"\t\t\t<input type=\"submit\" data-style=\"button save\" class=\"t_save\" id=\"save_options\" value=\"", trim($thesis->api->ef0(sprintf(__('Save %s', 'thesis'), $this->title))), "\" />\n",
...
*/
	public function _save($form) {
		...
		$box = array();
		if (is_array($options = $thesis->api->set_options(array_merge($this->_options(true), $this->_html_options(), $this->_html_admin(), $this->_uploader), array_merge($this->options, $values))))
			$box = $options;
		if ($this->name)
