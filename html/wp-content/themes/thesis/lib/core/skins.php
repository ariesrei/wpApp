...
/*
		...
		and then produces a useless Thesis installationcbecause there is no active Skin and
...
/*---:[ Thesis Admin methods for the Manage Skins page and associated functionality ]:---*/
	...
	public function canvas() {
		...
		$upload = current_user_can('manage_options') ?
			"<span id=\"skin_upload\" data-style=\"button action\">". __('Upload a New Skin', 'thesis'). "</span>" : false;
	...
	public static function item_info($skin, $active = false, $preview = false, $updates = array(), $depth = false) {
		...
		$zip = ($active || $preview) && apply_filters('thesis_skin_create_zip', false) ?
			"<a data-style=\"button action\" href=\"". wp_nonce_url(admin_url("update.php?action=thesis_generate_skin&skin=". esc_attr($class)), 'thesis-generate-skin'). "\">". __('Create Zip File', 'thesis'). "</a>" : false;
		$update = current_user_can('manage_options') && !empty($updates[$class]) && !empty($version) && version_compare($updates[$class]['version'], $version, '>') ?
			" <a onclick=\"if(!thesis_update_message()) return false;\" data-style=\"button update\" href=\"". wp_nonce_url(admin_url('update.php?action=thesis_update_objects&type=skin&class='. esc_attr($class). '&folder='. esc_attr($folder). '&version='. (!empty($updates[$class]['version']) ? esc_attr($updates[$class]['version']) : ''). '&name='. urlencode($name)), 'thesis-update-objects'). '">'. __('Update', 'thesis'). " $name</a>" : '';
		...
		return
			...
			"$tab\t\t<h4>$name $version <span class=\"skin_by\">". __('by', 'thesis'). "</span> <span class=\"skin_author\">". $thesis->api->efh($author). "</span></h4>\n".
			(!empty($update) ?
			"$tab\t\t<p>$update</p>\n" : '').
			(!empty($description) ?
			"$tab\t\t<p>". $thesis->api->efa($description). "</p>\n" : '').
			(!empty($docs) ?
			"$tab\t\t<p><a data-style=\"button\" href=\"". esc_url($docs). "\" target=\"_blank\" rel=\"noopener\">". __('See Documentation', 'thesis'). "</a></p>\n" : '').
			(($preview || ($active && !empty($zip)) || !($active || $preview)) ?
			"$tab\t\t<p>\n". (!($active || $preview) ?
			"$tab\t\t\t<input type=\"submit\" data-style=\"button action\" name=\"preview_skin\" value=\"". __('Preview Skin', 'thesis'). "\" />\n" : ($preview ?
			...
			(!$preview ?
			"$tab\t\t\t<button data-style=\"button delete\" class=\"skin_delete\" data-class=\"". esc_attr($class). "\" data-name=\"$name\">". __('Delete Skin', 'thesis'). "</button>\n" : '').
