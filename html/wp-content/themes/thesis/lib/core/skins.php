...
class thesis_skins {
	...
	private function skin() {
		...
		if (!empty($skin)) { // will only evaluate to false prior to initial installation
			...
			$skin_file = THESIS_USER_SKINS . "/{$this->skin['folder']}/skin.php";
			if (@file_exists($skin_file)) {
				...
				define('THESIS_USER_SKIN_URL', THESIS_USER_SKINS_URL . '/' . $this->skin['folder']);
	...
	public function get_items() {
		$skins = @scandir(THESIS_USER_SKINS);
		if (!@is_dir(THESIS_USER_SKINS) || !is_array($skins)) # if this is happening, the pooch has been completely sodomized
			return false;
		$installed = array();
		foreach ($skins as $skin) {
			$skin_file = THESIS_USER_SKINS . "/$skin/skin.php";
