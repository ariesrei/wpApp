...
final class thesis_skin_manager {
	...
	public $table;
	public $class;
	public $options = array('boxes', 'templates', 'packages', 'vars', 'css', 'custom_css');

	public function __construct($skin = array()) {
		global $wpdb, $thesis;
		if ($thesis->environment === false || !is_array($skin)) return;	// allow in all environments except the front end
		...
		$export = $thesis->api->form->fields(array(
			'export' => array(
				...
				'options' => array(
					...
					'custom_css' => sprintf(__('Custom %s', 'thesis'), $thesis->api->base['css'])),
				'default' => array(
					'boxes' => true,
					'templates' => true,
					'packages' => true,
					'vars' => true,
					'css' => true,
					'custom_css' => true))), array(), 'thesis_export_', '', 900, 6);
		$args = array(
	...
	public function defaults() {
		...
		elseif (($options = $thesis->api->verify_data_file(THESIS_USER_SKIN . '/seed.txt', $this->class)))
			foreach ($options as $name => $value)
				update_option($this->class . '_' . $name, $value);
		else
			return false;
		wp_cache_flush();
		return true;
