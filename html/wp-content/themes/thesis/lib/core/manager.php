		...
		foreach ($data as $option => $value)
			if (in_array($option, $this->options))
				if (!empty($value))
					...
					delete_option("{$this->class}_$option");
		wp_cache_flush();
		$thesis->skin->_write_css(!empty($data) && array_key_exists('_design', $data) && !empty($data['_design']) ? false : true);
		return true;
	}
...
/*---:[ Manager action helper methods ]:---*/
	...
	public function defaults($skin = array(), $restore = array(), $backup = false) {
		...
		if ($skin['class'] == $thesis->skin->_class)
			$thesis->skin->_write_css(!empty($restore) && array_key_exists('_design', $restore) ? false : true);
		return true;
	}
