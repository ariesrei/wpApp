...
class thesis_api {
	...
	public function get_option($option) {
		return isset($this->options[$option]) ? maybe_unserialize($this->options[$option]) : get_option($option);
	}
				...
				if ($field['type'] == 'group') {
					...
					if ($field['type'] == 'image') {
						...
						elseif ($field['type'] == 'image_upload') {
							if (!empty($value['url']) && !empty($value['height']) && !empty($value['width'])) {
	...
	public function save_image($location, $type = 'default', $post_id = 0) {
		...
		if ($type === 'default' || $type === 'box') {
			...
			if (empty($metadata) && empty($image_data))
				return false;
			$url = $post->guid;
		...
		if (!empty($return) && ((count($return) === 4 && in_array($type, array('default', 'box'))) || count($return) === 3))
			return $return;
		else return false;
	}
	...
	public function verify_data_file($file, $class, $type = 'skin') {
		...
		|| $unserialize['data']['class'] !== $class)
			return false;
		$options = $type == 'skin' ? array('boxes', 'templates', 'packages', 'vars', 'css', 'custom_css') : array();
		$real = array_intersect($options, array_keys($unserialize['data']));
		foreach ($real as $send)
			$data[$send] = $type == 'skin' && in_array($send, array('css', 'custom_css')) ? strip_tags(stripslashes($unserialize['data'][$send])) : stripslashes_deep($unserialize['data'][$send]);
		return $data;
	}
	...
	private function strings() {
		...
		$strings = array(
			...
			'comment_permalink' => __('permalink to this comment', 'thesis'));
		return /*!$thesis->environment && !is_admin() ? $strings : */array_merge($strings, array(
			'save' => __('Save', 'thesis'),
