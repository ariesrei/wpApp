...
/*---:[ Skin Content page (with Display options) ]:---*/
	...
	public function _content_admin() {
		...
		echo
			...
			$display['output'].
			"\t\t\t<input type=\"submit\" data-style=\"button save\" class=\"t_save\" id=\"save_options\" value=\"". $thesis->api->ef0(__('Save Display Options', 'thesis')). "\" />\n".
...
/*---:[ Skin Design page ]:---*/
	...
	public function _design_admin() {
		...
		echo
			...
			$options['output'],
			"\t\t\t</div>\n",
			"\t\t\t<input type=\"submit\" data-style=\"button save\" class=\"t_save\" id=\"save_options\" value=\"", $thesis->api->ef0(__('Save Design Options', 'thesis')), "\" />\n",
...
/*---:[ Thesis Skin Editor ]:---*/
	...
	private function _launch_canvas() {
		...
		$url = $real_scheme. '://'. (!empty($host) ? trailingslashit($host) : ''). (!empty($path) && $path != '/' ? trailingslashit($path) : '' ). '?'. (! empty($query) ? rtrim($query, '&'). '&' : ''). 'thesis_canvas=1&thesis_canvas_nonce='. wp_create_nonce('thesis-canvas-url'). (!empty($fragment) ? "#$fragment" : '');
...
/*---:[ ajax interface actions ]:---*/
	...
	public function _write_css($design = false) {
