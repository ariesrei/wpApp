...
class thesis_skin {
	...
	private function _launch_canvas() {
		...
		$parsed = parse_url((!empty($_SERVER['HTTP_REFERER']) && strpos(strtolower($_SERVER['HTTP_REFERER']), strtolower(admin_url())) !== 0 ? $_SERVER['HTTP_REFERER'] : site_url('', $real_scheme)));
		extract($parsed);
...
<?php
	...
	public function _editor_launcher() {
		...
		echo
			...
			"\t<form method=\"post\" action=\"" . site_url('?thesis_editor=1', $scheme) . "\">\n".
	...
	public function _query($query) {
		...
		$custom = false;
		if ($query->is_page && !empty($query->queried_object) && is_object($query->queried_object)) {
			$redirect = ($redirect = get_post_meta($query->queried_object->ID, '_thesis_redirect', true)) ? $redirect : false; #wp
			if (is_array($redirect) && !empty($redirect['url'])) wp_redirect($redirect['url'], 301); #wp
			$custom = is_array($post_meta = get_post_meta($query->queried_object->ID, "_{$this->_class}", true)) ? (!empty($post_meta['template']) ? $post_meta['template'] : false) : false;
		}
	...
	private function _editor() {
		...
		$menu =
			"<ul id=\"t_menu\">\n".
			$li.
			"\t<li class=\"t_logo t_right\"><a href=\"" . esc_url(admin_url('admin.php?page=thesis')) . "\">Thesis</a></li>\n".
			"\t<li class=\"t_right\"><a class=\"t_menu_link\" href=\"" . esc_url(site_url()) . '">' . __('View Site', 'thesis') . "</a></li>\n".
			"</ul>\n";
		echo
			"<!DOCTYPE html>\n".
