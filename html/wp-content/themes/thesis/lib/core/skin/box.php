...
class thesis_box {
	...
	public function __construct($box = array()) {
		...
		$this->_admin = is_array($admin = $this->_admin()) ? $admin : $this->_admin;
		...
		if (method_exists($this, 'site_menu'))
			add_filter('thesis_site_menu', array($this, '_add_site_menu'));
		if (method_exists($this, 'boxes_menu'))
			add_filter('thesis_boxes_menu', array($this, '_add_boxes_menu'));
		...
		if (method_exists($this, 'uploader'))
			$this->uploader();
		if ((!empty($check) && !$this->name) || empty($check))
			$this->construct();
	}
	...
	public function _add_site_menu($site) {
		return is_array($menu = $this->site_menu()) ? (is_array($site) ? array_merge($site, $menu) : $menu) : $site;
	}

	public function _add_boxes_menu($boxes) {
		return is_array($menu = $this->boxes_menu()) ? (is_array($boxes) ? array_merge($boxes, $menu) : $menu) : $boxes;
	}
	...
	public function _save($form) {
		...
		$box = array();
		if (is_array($options = $thesis->api->set_options(array_merge($this->_get_options(), $this->_admin), !empty($values) ? $values : array())))
	...
	protected function rotator($depth) {
		global $thesis;
...
class thesis_image_uploader_box extends thesis_box {
	protected function uploader() {
		new thesis_upload($args = array(
			'title' => __('Upload Image', 'thesis'),
			'prefix' => $this->_class,
			'file_type' => 'image',
			'folder' => 'box'));
		add_action("{$args['prefix']}_before_thesis_iframe_form", array($this, '_script'));
	}
	...
	public function _script() {
		...
		if (!empty($_GET['url']) && !empty($_GET['height']) && !empty($_GET['width']))
			echo
				"<script type=\"text/javascript\">".
					"function fill_inputs(){".
						"parent.document.getElementById('". $this->_class. '_' .$this->_id ."_image_url').value = '". esc_url(urldecode($_GET['url'])) ."';".
						"parent.document.getElementById('". $this->_class. '_' .$this->_id ."_image_height').value = '". (int)$_GET['height'] ."';".
						"parent.document.getElementById('". $this->_class. '_' .$this->_id ."_image_width').value = '". (int)$_GET['width'] ."';".
					"}".
					"fill_inputs();".
				"</script>";
	}
...
class thesis_favicon extends thesis_image_uploader_box {
	...
	public function html() {
		$url = esc_url(empty($this->options['image']) || !is_array($this->options['image']) ?
...
class thesis_wp_nav_menu extends thesis_box {
	...
	public function html($depth) {
		echo str_repeat("\t", $depth) . wp_nav_menu(array_merge($this->options, array('container' => false, 'echo' => false))) . "\n"; #wp
	}
}
...
class thesis_post_box extends thesis_box {
	...
	public function html($depth, $post_count = false) {
	...
	protected function options() {
		...
		return array(
			...
			'class' => array(
				...
				'width' => 'medium',
				'label' => $thesis->api->strings['html_class'],
				'tooltip' => __('This box already contains a <abbr title="Cascading Style Sheet">CSS</abbr> class called <code>headline</code>. If you want to add an additional <abbr title="Cascading Style Sheet">CSS</abbr> class, you should enter that here. Separate multiple classes with spaces.<br /><br /><strong>Note</strong>: class names cannot begin with numbers!', 'thesis')));
	}

	public function html($depth, $schema = false) {
		$html = !empty($this->options['html']) ? $this->options['html'] : 'h1';
	 	echo
			str_repeat("\t", $depth).
			"<$html class=\"headline\"" . ($schema ? ' itemprop="name"' : '') . '>'.
...
class thesis_post_author extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		$author = !empty($this->options['link']['on']) ?
			...
			get_the_author(); #wp
		echo
			str_repeat("\t", $depth) . (!empty($this->options['intro']) ?
...
/*			'default_avatar' => array(
	...
	public function html($depth, $schema = false) {
		global $post;
		echo str_repeat("\t", $depth) . get_avatar(
			$post->post_author,
...
class thesis_post_date extends thesis_box {
	...
	protected function options() {
		...
		return array(
			...
			'class' => array(
				...
				'code' => true,
				'label' => $thesis->api->strings['html_class'],
				'tooltip' => sprintf(__('This box already contains a %1$as of <code>post_date</code>. If you&#8217;d like to supply another %1$s, you can do that here.%2$s', 'thesis'), $thesis->api->base['class'], $thesis->api->strings['class_note'])),
			'schema' => array(
				'type' => 'checkbox',
	...
	public function html($depth, $schema = false) {
		...
		echo
			...
			(!$schema || ($schema && !isset($this->options['schema']['only'])) ?
...
class thesis_post_excerpt extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		$tab = str_repeat("\t", $depth);
		echo
			"$tab<div class=\"post_excerpt" . (!empty($this->options['class']) ? ' ' . trim($thesis->api->esc($this->options['class'])) : '') . '"' . ($schema ? ' itemprop="description"' : '') . ">\n".
...
class thesis_post_categories extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		echo
			"$tab<$html class=\"post_cats\"" . ($schema ? ' itemprop="keywords"' : '') . ">\n".
			(!empty($this->options['intro']) ?
...
class thesis_post_tags extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		echo
			"$tab<$html class=\"post_tags\"" . ($schema ? ' itemprop="keywords"' : '') . ">\n".
			(!empty($this->options['intro']) ?
...
class thesis_post_image extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		$tab = str_repeat("\t", $depth);
		$attachment = get_post($this->post_meta['image']['id']);
		$alt = !empty($this->post_meta['alt']) ?
			$this->post_meta['alt'] : ($wp_alt = get_post_meta($this->post_meta['image']['id'], '_wp_attachment_image_alt', true) ?
		...
		if (!empty($this->post_meta['image']['url']))	
			$img = "<img class=\"post_image$alignment$frame\" src=\"". esc_url($this->post_meta['image']['url']) ."\"$dimensions alt=\"" . trim($thesis->api->escht($alt, true)) . "\"" . ($schema ? ' itemprop="image"' : '') . " />";
	...
	public function add_image_to_feed($content) {
		...
		$attachment = &get_post($this->post_meta['image']['id']); #wp
		$alt = !empty($this->post_meta['alt']) ?
			$this->post_meta['alt'] : ($wp_alt = get_post_meta($this->post_meta['image']['id'], '_wp_attachment_image_alt', true) ?
			$wp_alt : get_the_title() . ' ' . strtolower($this->image_type));
		$caption = !empty($this->post_meta['caption']) ?
...
class thesis_post_thumbnail extends thesis_box {
	...
	public function html($depth, $schema = false) {
		...
		$img = '';
		if (!empty($this->post_meta['image']['url']))	
			$img = "<img class=\"thumb$alignment$frame\" src=\"". esc_url($this->post_meta['image']['url']) ."\"$dimensions alt=\"" . trim($thesis->api->escht($alt, true)) . '"' . ($schema ? ' itemprop="thumbnailUrl"' : '') . " />";
...
class thesis_comments extends thesis_box {
	...
	protected function options() {
		...
		return array(
			'html' => array(
				'type' => 'select',
				'label' => __('Comment Wrapper', 'thesis') . " {$thesis->api->strings['html_tag']}",
				'tooltip' => __('Select the comment markup structure.', 'thesis'),
				'options' => array(
					'ul' => 'ul',
					'ol' => 'ol',
					'div' => 'div'),
			...
			'id' => array(
				...
				'width' => 'medium',
				'code' => true,
				'label' => __('Comment Wrapper <code>id</code>', 'thesis'),
				'tooltip' => __('The default ID for the comment wrapper is <code>comments</code>. If you&#8217;d like to use a different ID, you can supply it here.', 'thesis'),
				'placeholder' => 'comments'),
			'class' => array(
				'type' => 'text',
				'width' => 'medium',
				'code' => true,
				'label' => __('Comment Wrapper <code>class</code>', 'thesis'),
				'tooltip' => __('If you like, you can add an HTML <code>class</code> to the markup you selected above.', 'thesis')),
			'per_page' => array(
				'type' => 'text',
				'width' => 'tiny',
	...
	public function html($depth) {
		global $thesis, $wp_query;
		$tab = str_repeat("\t", ($this->tab_depth = $depth));
		if ($this->abort === false) {
	...
	public function start($comment, $args, $depth) {
		...
		$this->rotator($this->tab_depth + 2);
	}
}

// override the comment_walker insanity
...
class thesis_comment_edit extends thesis_box {
	...
	public function html($depth) {
		global $thesis;
		if ($link = get_edit_comment_link())
			echo
				str_repeat("\t", $depth).
				"<a class=\"comment_edit\" href=\"$link\" rel=\"nofollow\">" . trim(esc_html(apply_filters($this->_class, strtolower($thesis->api->strings['edit'])))) . "</a>\n";
	}
}
...
class thesis_comment_form extends thesis_box {
	...
	public function html($depth) {
		...
		if (get_option('comment_registration') && !!!$user_ID) #wp
			...
			$this->rotator($depth + 1, get_option('require_name_email'));
...
class thesis_comment_form_name extends thesis_box {
	...
	public function html($depth, $req) {
		...
		if (!!$user_ID) // This should probably be moved to the comment form box to safeguard against unwanted display outcomes
			echo
				"$tab<p>" . __('Logged in as', 'thesis') . ' <a href="' . site_url('wp-admin/profile.php') . "\" rel=\"nofollow\">$user_identity</a>. ".
...
class thesis_previous_post_link extends thesis_box {
	...
	public function html($depth) {
		...
		echo str_repeat("\t", $depth) . "<$html class=\"previous_post\">";
...
class thesis_next_post_link extends thesis_box {
	...
	public function html($depth) {
		...
		$html = !empty($this->options['html']) ? $this->options['html'] : 'span';
		echo str_repeat("\t", $depth) . "<$html class=\"next_post\">";
...
class thesis_previous_posts_link extends thesis_box {
	...
	public function html($depth) {
		...
		$html = !empty($this->options['html']) ? $this->options['html'] : 'span';
		echo
			str_repeat("\t", $depth) . "<$html class=\"previous_posts\">".
...
class thesis_next_posts_link extends thesis_box {
	...
	public function html($depth) {
		...
		$html = !empty($this->options['html']) ? $this->options['html'] : 'span';
		echo
			str_repeat("\t", $depth) . "<$html class=\"next_posts\">".
...
class thesis_archive_title extends thesis_box {
	...
	public function html($depth) {
		...
		if ($title)
			echo str_repeat("\t", $depth) . "<h1 class=\"archive_title\">" . trim(apply_filters($this->_class, $title)) . "</h1>\n";
	}
}
	...
	protected function options() {
		...
		return array(
			...
			'html' => array(
				...
				'options' => array(
					'div' => 'div',
					'none' => sprintf(__('No %s wrapper', 'thesis'), $thesis->api->base['html'])),
				'default' => 'div',
				'dependents' => array(
					'div' => true)),
			'id' => array(
...
class thesis_wp_admin extends thesis_box {
	...
	public function html($depth) {
		global $thesis;
		echo str_repeat("\t", $depth) . "<p><a href=\"" . admin_url() . '">' . sprintf(__('%s Admin', 'thesis'), $thesis->api->base['wp']) . "</a></p>\n"; #wp
	}
}
...
/*		$more['year'] = array(
		...
		$more['month'] = array(
			...
			'options' => array(
				...
				'7' => __('July', 'thesis'),
				'8' => __('August', 'thesis'),
				'9' => __('September', 'thesis'),
				'10' => __('October', 'thesis'),
				'11' => __('November', 'thesis'),
				'12' => __('December', 'thesis')),
			);
		...
		$options['html'] = array(
			'type' => 'group',
			'label' => sprintf(__('Query Box %s Options', 'thesis'), $thesis->api->base['html']),
			'fields' => array(
				'tag' => array(
					'type' => 'select',
					'label' => $thesis->api->strings['html_tag'],
					'tooltip' => __('Your selection here will affect the query box output. Choose between <code>div</code>s or unordered/ordered lists.', 'thesis'),
					'options' => array(
						'div' => __('div', 'thesis'),
						'ul' => __('ul', 'thesis'),
						'ol' => __('ol', 'thesis')),
				...
				'output' => array(
					...
					'tooltip' => __('Selecting this will link the output to the post permalink. All output will be linked.', 'thesis'),
					'options' => array(
						'link' => __('Link query output to post permalink', 'thesis')),
					'parent' => array(
						'tag' => array('ul', 'ol'))),
				'id' => array( // Ultimately, the ID and class used on either the div or list should be the same...
					'type' => 'text',
					'width' => 'medium',
					'code' => true,
					'label' => $thesis->api->strings['html_id'],
				...
				'hook' => array(
					...
					'code' => true,
					'label' => $thesis->api->strings['hook_label'],
					'tooltip' => $thesis->api->strings['hook_tooltip_1'] . '<br /><br /><code>thesis_hook_before_query_box_{name}</code><br /><code>thesis_hook_query_box_{name}_top</code><br /><code>thesis_hook_query_box_{name}_bottom</code><br /><code>thesis_hook_after_query_box_{name}</code><br /><br />' . $thesis->api->strings['hook_tooltip_2'])));
		return $options;
	}
	...
	public function html($depth) {
		...
		$tag = !empty($this->options['tag']) && ($this->options['tag'] == 'ul' || $this->options['tag'] == 'ol') ? $this->options['tag'] : 'div'; // list or post output
		$list = $tag == 'ul' || $tag == 'ol' ? true : false;
		$link = !empty($this->options['output']['link']) ? $this->options['output']['link'] : false;
		$id = !empty($this->options['id']) ? ' id="' . trim($this->options['id']) . '"' : '';
		$class = $list ?
			'query_list' . (!empty($this->options['list_class']) ? ' ' . trim($this->options['list_class']) : '') :
			'query_box' . (!empty($this->options['class']) ? ' ' . trim($this->options['class']) : '') . (!empty($this->options['wp']['auto']) ? implode(' ', get_post_class()) : ''); #wp
		$hook = !$list && !empty($this->options['hook']) ? "_{$this->options['hook']}" : '';
		$counter = 1;
		$depth = $list ? $depth + 2 : $depth + 1;
		if (!!$list) {
			do_action("thesis_hook_before_query_list$hook");
			echo "$tab<$tag$id class=\"$class\">\n";
		...
		while ($the_query->have_posts()) {
			...
			$this->rotator($depth, $counter, $list); // let all the dependents know we're doing a list or not
			if (!!$list) {
				echo ($link ?
					"$tab\t\t</a>\n" : '').
					"$tab\t</li>\n";
...
class thesis_js extends thesis_box {
	...
	public function head_scripts() {
		if (empty($this->template_options['head_scripts'])) return;
...
class thesis_tracking_scripts extends thesis_box {
	...
	public function construct() {
		...
		if ($this->analytics && !is_admin())
			add_action('_thesis_analytics', array($this, 'analytics'));
		if ($this->scripts && !is_admin())
			add_action('_thesis_tracking_scripts', array($this, 'scripts'));
		if ($thesis->environment == 'admin' && !empty($_GET['canvas']) && $_GET['canvas'] == 'tracking') {
	...
	protected function site_menu() {
		global $thesis;
		return array('tracking' => array(
			'text' => $thesis->api->strings['tracking_scripts'],
			'url' => admin_url('admin.php?page=thesis&canvas=tracking')));
	}
...
class thesis_404 extends thesis_box {
	...
	public function construct() {
		...
		if (($this->page = is_numeric($page = $thesis->api->get_option('thesis_404')) ? $page : $this->page) && !is_admin())
			add_filter('thesis_404', array($this, 'query'));
		if ($thesis->environment == 'admin' && !empty($_GET['canvas']) && $_GET['canvas'] == '404page') {
			add_action('thesis_admin_canvas', array($this, 'canvas'));
	...
	protected function site_menu() {
		global $thesis;
		return array('404page' => array(
			'text' => $this->title,
			'url' => admin_url('admin.php?page=thesis&canvas=404page')));
	}
