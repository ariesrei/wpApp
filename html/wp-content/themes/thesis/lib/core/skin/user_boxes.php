...
class thesis_user_boxes {
	...
	public function canvas() {
		...
		echo
			...
			(current_user_can('manage_options') ? " <span id=\"object_upload\" data-style=\"button action\" title=\"". __('upload a new box', 'thesis'). "\">". __('Upload a New Box', 'thesis'). "</span>" : ''),
			"</h3>\n",
	...
	public static function item_info($box, $active = array(), $updates = array(), $depth = 0) {
		...
		$settings = !empty($box['settings']) ?
			'<a data-style="button action" href="'. admin_url($box['settings']). '">'. __('Settings', 'thesis'). '</a>' : '';
		$docs = !empty($box['docs']) ?
			'<a data-style="button" href="'. esc_url($box['docs']). '" target="_blank" rel="noopener">'. __('See Documentation', 'thesis'). '</a>' : '';
		$update = !empty($updates[$box['class']]) && version_compare($updates[$box['class']]['version'], $box['version'], '>') ?
			"<a onclick=\"if(!thesis_update_message()) return false;\" data-style=\"button update\" href=\"". wp_nonce_url(admin_url("update.php?action=thesis_update_objects&type=box&class=$id&name=". urlencode($thesis->api->efh($box['name']))), 'thesis-update-objects'). '">'. sprintf(__('Update %s', 'thesis'), $thesis->api->efh($box['name'])). '</a>' : '';
		return
			"$tab\t\t<div id=\"box_$id\" class=\"object". (!empty($checked) ? ' active_object' : ''). "\">\n".
			"$tab\t\t\t<label for=\"$id\">". $thesis->api->efh($box['name']). " $version$author</label>\n".
			"$tab\t\t\t<p class=\"object_description\">". $thesis->api->efa($box['description']). "</p>\n".
			(!empty($update) || !empty($settings) || !empty($docs) ?
			"$tab\t\t\t<p>". (!empty($update) ? $update : ''). (!empty($settings) ? $settings : ''). (!empty($docs) ? $docs : ''). "</p>\n" :'').
			"$tab\t\t\t<input type=\"checkbox\" class=\"select_object\" id=\"$id\" name=\"boxes[$id]\" value=\"1\"$checked />\n".
			"$tab\t\t\t<button data-style=\"button delete\" class=\"delete_object\" data-type=\"box\" data-class=\"$id\" data-name=\"". $thesis->api->efh($box['name']). "\">". __('Delete Box', 'thesis'). "</button>\n".
			"$tab\t\t</div>\n";
	}
