...
class thesis_packages {
	...
	private function available($user) {
		$core = is_array($classes = apply_filters('thesis_core_packages', $this->core)) ? $classes : $this->core;
	...
	public function edit() {
		...
		elseif (is_object($this->active[$pkg['id']]))
	...
	public function css($css) {
		...
		$clearfix = array();
		foreach ($this->active as $id => $pkg) {
			$css = preg_replace('/&' . $pkg->options['_ref'] . '((?=[\s\r\n])|\Z)/i', stripslashes($pkg->css() . (!empty($pkg->options['_css']) ? "\n" . trim($pkg->options['_css']) : '')), $css, 1);
