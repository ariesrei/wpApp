...
class thesis_meta_robots extends thesis_box {
	...
	public function html() {
		global $thesis, $wp_query;
		if ($thesis->api->get_option('blog_public') == 0) return;
		$content = array();
...
class thesis_stylesheets_link extends thesis_box {
	...
	public function html() {
		...
		— NOTE: This is an array filter, and it can be used to output more stylesheets after the main one
		*/
...
class thesis_canonical_link extends thesis_box {
	...
	public function html() {
		...
		if (!empty($url))
			echo "<link rel=\"canonical\" href=\"", esc_url(apply_filters($this->_class, $url)), "\" />\n";
	}
}
...
class thesis_favicon extends thesis_box {
	...
	protected function translate() {
		...
		$this->tooltip = sprintf(__('If you don&#39;t already have a favicon, you can generator one with this handy <a href="%1$s" target="_blank" rel="noopener">online tool</a>.', 'thesis'), 'http://favicon-generator.org/');
...
class thesis_attribution extends thesis_box {
	...
	public function html($args = array()) {
		...
		if (!empty($this->options['text']))
			...
			$text = sprintf(__('This site rocks the %1$s Skin for <a href="%2$s">Thesis</a>.', 'thesis'), $skin, esc_url(apply_filters("{$this->_class}_url", 'http://diythemes.com/')));
		}
		echo
			str_repeat("\t", !empty($depth) ? $depth : 0),
...
class thesis_404 extends thesis_box {
	...
	public function admin() {
		...
		$docs = !empty($this->filters['docs']) ?
			' <a data-style="button" href="'. esc_url($this->filters['docs']). '" target="_blank" rel="noopener">'. __('See Documentation', 'thesis'). '</a>' : '';
		echo
			...
			"$tab\t<input type=\"submit\" data-style=\"button save\" class=\"t_save\" id=\"save_options\" value=\"", trim($thesis->api->ef0(sprintf(__('%1$s %2$s', 'thesis'), __($thesis->api->strings['save'], 'thesis'), $this->title))), "\" />\n",
			"$tab</form>\n",
			"$tab<a id=\"edit_404\" data-style=\"button action\" href=\"", admin_url("post.php?post=$this->page&action=edit"), "\" data-base=\"", admin_url('post.php?post='), "\">", trim($thesis->api->efh(sprintf(__('%1$s %2$s', 'thesis'), __($thesis->api->strings['edit'], 'thesis'), $this->title))), "</a>\n";
	}
